...
def load_data(user,item):
    ...
    support_item = []
    # rating matrix
    with open('./data/rate_matrix.pkl','rb') as source:#换成rate_matrix要加todense()，换回来要删去todense()
        rating = pkl.load(source)
    # concept w2v features
    with open('./data/concept_embedding.pkl','rb') as source:
        concept_w2v = np.array(pkl.load(source))
    # concept bow features
    #with open('./data/concept_feature_bow.pkl','rb') as source:
        #concept_bow = pkl.load(source).todense()
        # print(concept_bow.shape)
    # concept = np.hstack((concept_w2v, concept_bow))
    #concept = concept_w2v
    features_item = preprocess_features(concept_w2v.astype(np.float32))
    # user features
    with open('./data/UC.pkl','rb') as source:
        features = pkl.load(source)
    features_user = preprocess_features(features.astype(np.float32))
    # uku
    if 'uku' in user or 'kuk' in item:
        with open('./data/adjacency_matrix.pkl','rb') as source:
            uk = pkl.load(source)
        if 'uku' in user:
    ...
    # uctcu
    if 'uctcu' in user:
        with open('./data/UCT.pkl','rb') as source:
            uct = pkl.load(source)
